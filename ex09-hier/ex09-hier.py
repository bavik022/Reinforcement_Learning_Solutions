...
def plot_termination_maps(env, terminations):
    ...
    termination_maps = [env.occupancy.astype(
        'float64') for _ in range(len(terminations))]
    for option in range(len(terminations)):
        state = 0
        for i in range(13):
            for j in range(13):
                if termination_maps[option][i, j] == 0:
                    termination_maps[option][i,
                                             j] = terminations[option].pmf(state)
                    state += 1
...
def option_critic(env):
    ...
    option_policies = [SoftmaxPolicy(
        nstates, nactions, temperature) for _ in range(noptions)]
    option_terminations = [SigmoidTermination(
        nstates) for _ in range(noptions)]
    ...
    for episode in range(nepisodes):
        ...
        for step in range(nsteps):
            ...
            # perform action: observe nextstate, reward
            ...
            if not done:
                beta = option_terminations[option].sample(nextstate)
                maxQ = np.max(Q_omega[nextstate])
                delta = delta + (gamma * (1 - beta) *
                          Q_omega[nextstate, option]) + (gamma * beta * maxQ)

            Q_U[state, option, action] = Q_U[state, option, action] + alpha*delta
            ...
            loggrad = option_policies[option].loggradient(state, action)
            option_policies[option].weights =  option_policies[option].weights + lr_intra * \
                loggrad * Q_U[state, option, action]

            grad = option_terminations[option].gradient(nextstate)
            V = np.max(Q_omega[nextstate])
            option_terminations[option].weights = option_terminations[option].weights - lr_term * \
                grad * (Q_omega[nextstate, option] - V)
